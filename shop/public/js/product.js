(function () {
    ...
    const utils = {
        ...
        buildUrl: ({ categoryId, color, minPrice, maxPrice, sort, search, page, size }) => {
            const params = new URLSearchParams();
            params.append('size', size || CONSTANTS.PAGE_SIZE);

            if (page) params.append('page', page);
            if (categoryId) {
                params.append('categoryId', categoryId.length > 1 ? categoryId : `[startsWith]${categoryId}`);
            }
            if (color && color !== "default") params.append('color', `[like]${color}`);
        ...
        findCorrespondingFilterButton: (categoryId) => {
            return Array.from(DOM.categoryButtons).find(button => {
                ...
                return buttonFilter === categoryId ||
                    (buttonFilter === categoryId.charAt(0) && categoryId.length > 1);
            });
        }
    };
    ...
    const renderer = {
                    ...
                    <div class="block2">
                        ...
                        <div class="block2-txt flex-w flex-t p-t-14">
                            <div class="block2-txt-child1 flex-col-l">
                                ...
                                <span class="stext-105 cl3">${utils.formatCurrency(product.price, ' VND', 2)} </span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        renderPagination: (totalPages, currentPage) => {
            ...
            const createPageItem = (pageNum, isActive = false, isDisabled = false, text = pageNum) => `
            ...
            const items = [];
            items.push(createPageItem(currentPage - 1, false, currentPage === 1, 'Prev'));
            items.push(createPageItem(1, currentPage === 1));

            if (currentPage > 5) items.push('<li class="page-item disabled"><span class="page-link">...</span></li>');

            for (let i = Math.max(2, currentPage - 3); i <= Math.min(totalPages - 1, currentPage + 2); i++) {
                items.push(createPageItem(i, i === currentPage));
            }

            if (currentPage < totalPages - 3) items.push('<li class="page-item disabled"><span class="page-link">...</span></li>');

            if (totalPages > 1) items.push(createPageItem(totalPages, currentPage === totalPages));
            items.push(createPageItem(currentPage + 1, false, currentPage === totalPages, 'Next'));
    ...
    const ui = {
        ...
        updateActiveStates: (elements, activeElement, activeClass = 'filter-link-active') => {
        ...
        syncFilterButtonWithCategory: (categoryId) => {
            ...
            DOM.categoryButtons.forEach(btn => btn.classList.remove('how-active1'));

            // Find and activate the corresponding filter button
            const correspondingButton = utils.findCorrespondingFilterButton(categoryId);
    ...
    async function fetchAndRenderProducts(params = {}) {
        ...
        try {
            ...
            if (params.categoryId !== undefined) STATE.selectedCategory = params.categoryId;
            ...
            const currentItems = DOM.productsContainer.querySelectorAll('.isotope-item');
    ...
    function initializeEventListeners() {
        ...
        DOM.categoryButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                ui.updateActiveStates(DOM.categoryButtons, button, 'how-active1');
        ...
        DOM.colorLinks.forEach(link => {
            link.addEventListener('click', (e) => {
                ...
                ui.updateActiveStates(DOM.colorLinks, link, 'filter-link-active');
            });
        });
        ...
        DOM.priceLinks.forEach(link => {
            link.addEventListener('click', (e) => {
                e.preventDefault();
                STATE.selectedPriceRange = link.getAttribute('data-price-range');
        ...
        DOM.paginationContainer.addEventListener('click', (e) => {
            ...
            if (!isNaN(page) && page > 0 && page <= STATE.totalPages && currentPage != page) {
