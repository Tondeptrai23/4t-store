...
class ProductController {
    async getAll(req, res, next) {
        try {
            ...
            const data = await productService.getFilteredSortedAndPaginatedProducts(req.query);
            const products = data.products || [];
            const page = data.pagination.page || 1;
            const limit = data.pagination.limit || 8;
            const totalProducts = data.count || products.length; // Tổng số sản phẩm
            const totalPages = data.pagination.totalPages || Math.ceil(totalProducts / limit);
            ...
            res.render('index', { body: 'pages/productlist', products: formattedProducts, categories, subcategories, totalPages, currentPage: page, isLoggedIn: isLoggedIn});
