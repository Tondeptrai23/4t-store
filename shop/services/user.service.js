...
class UserService {
        ...
        try {
            ...
            if (existingUser) {
                if (existingUser.deletedAt !== null) {
                    console.log(
                        "User exists and is soft-deleted, creating a new user with the same email"
                    );

                    throw new Error(
                        "Email này đã được đăng ký và bị xóa. Không thể phục hồi!"
                    );
                }
    ...
    getFilteredSortedAndPaginatedUsers = async (requestQuery) => {
        ...
        try {
            ...
            console.error("Error in getFilteredSortedAndPaginatedUsers:", err);
